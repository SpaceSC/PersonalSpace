...
it("checks if a get request to /api/random-apod returns status(200) and a random APOD", async () => {
  // given
...
it("checks if a get request to /api/random-apod returns the err.status when the date is invalid", async () => {
  // given
  const token = jwt.sign({ google_id: 1 }, process.env.JWT_SECRET);
...
it("checks if a get request to /api/random-apod returns status(503) and the message 'Houston, we've had a problem' when the date is invalid", async () => {
  // given
  const token = jwt.sign({ google_id: 1 }, process.env.JWT_SECRET);
