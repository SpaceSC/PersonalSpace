...
it("checks if a get request to /api/apod returns status(200) and a random APOD", async () => {
  // given
  ...
  // when
  const response = await request.get("/api/apod").set("Authorization", `Bearer ${token}`);
...
it("checks if a get request to /api/apod returns returns status(400) when the date is invalid", async () => {
  const token = jwt.sign({ google_id: 1 }, process.env.JWT_SECRET);
...
it("checks if a get request to /api/apod returns status(503) and the message 'Houston, we've had a problem' when today's APOD hasn't been uploaded in the US", async () => {
  // given
...
//   // when
//   const response = await request.get("/api/apod").set({authorization: token});
